...
readonly SUBNET_ID=$(aws ec2 describe-subnets --output json --filters "Name=tag:kubernetes.io/cluster/${CLUSTER_NAME},Values=owned" | jq -r --arg key "${CLUSTER_NAME}-public" '.Subnets[] | select((.Tags[].Value | contains($key)) and (.Tags[].Value | endswith("a"))) | .SubnetId')
