...
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -n core@${running_build_node} "sudo dd if=/dev/zero of=file$(date +"%s").txt bs=1048576 count=1024"
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -n core@${running_build_node} "sudo df -h"
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -n core@${running_build_node} "sudo ls -al -h"
oc get pod -o wide -n testproject
oc get build -n testproject
